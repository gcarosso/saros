...
const App={tab:'cal',tlMode:'ph',calSort:['pd',1],calPage:0,calDense:0,spN:25,fdaF:'nme',fdaPage:0,fdaSort:['d',-1],
  ...
  /* ---- readout calendar ---- */
  renderCal(){
    const cols=[['★','wl',0,'wl'],['Primary completion','pd',1,'readout'],['Δ days','days',1,'days'],['Phase','ph',1,'phase'],['Asset / study','t',0],['Sponsor','grp',1,'sponsor'],['Indication','c',0],['Area','ta',1,'ta'],['Modality','mo',1,'mo'],['Status','st',1,'status'],['N','n',1,'n'],['Conf.','conf',1,'conf'],['Prior','pri',1,'prior'],['Impact','impact',1,'impact'],['Flag','action',1,'action'],['Mkt cap','mcap',1,'mcap']];
    ...
    $('#caltbl tbody').innerHTML=slice.map(t=>`<tr class="r${Dossier.cur&&Dossier.cur.id===t.id?' sel':''}" data-id="${t.id}" style="${App.calDense?'font-size:11px':''}">
      ...
      <td><span class="act ${t.flagClass}" title="${esc(t.actionWhy)}">${esc(t.action)}</span></td>
      <td class="num dim" title="${t.mcapSrc==='price'?'last close × shares outstanding':t.mcapSrc==='float'?'SEC public float (no price)':''}">${t.mcap?fmtUSD(t.mcap)+(t.mcapSrc==='float'?'<span class="dim2"> f</span>':''):'—'}</td></tr>`).join('')||`<tr><td colspan="16" class="empty">${F.wl&&!WL.s.size?'Your book is empty. Open a trial and press ★ (or W) to add it; the Book preset then shows only those.':'No trials match. Loosen a filter or clear the focus.'}</td></tr>`;
    bindTips($('#caltbl'));
  ...
  renderRisk(){
    ...
    $('#risktbl thead').innerHTML='<tr><th>Primary completion</th><th>Δ days</th><th>Asset / study</th><th>Sponsor</th><th>Indication</th><th>Status</th><th class="num">N</th><th class="num">Conf.</th><th>Why</th></tr>';
  ...
  renderAbout(){
    const ST=DATA.status||{};const stRows=Object.entries(ST).map(([k,v])=>`<tr><td>${esc(v.source||k)}</td><td class="num">${v.pulled?esc(String(v.pulled).slice(0,16).replace('T',' '))+'Z':'<span class="dim2">never</span>'}</td><td class="num">${(v.rows||0).toLocaleString()}</td></tr>`).join('');
    $('#prov').innerHTML=`<div class="lbl" style="margin-bottom:6px">Sources status <span style="text-transform:none;letter-spacing:0">· what each loader last pulled and how many rows reached this build · build ${esc(META.built||'')}</span></div><table class="t" style="margin-bottom:14px"><thead><tr><th>Source</th><th class="num">Last pulled</th><th class="num">Rows</th></tr></thead><tbody>${stRows}</tbody></table><div class="prov">source        ClinicalTrials.gov API v2 · GET /api/v2/studies
...
function cmp(a,b,k){let x=a[k],y=b[k];if(k==='mcap'){x=a.mcap;y=b.mcap;}if(k==='action'){const o=['Stopped','Cash flag','Date passed','Date unconfirmed','Date slipped','Specialist adds','Insider buys','Enrollment lag','Pivotal, actual date','Pivotal inside 6 mo','No flag'];return (o.indexOf(x)-o.indexOf(y));}if(false){const o=['Pass','Cash flag','Wait','Review flow','Diligence enrollment','Size / diligence endpoints','Diligence endpoints','Monitor'];x=o.indexOf(a.action);y=o.indexOf(b.action);}if(k==='wl'){x=WL.has(a.id)?1:0;y=WL.has(b.id)?1:0;}if(k==='c'){x=a.c[0]||'';y=b.c[0]||'';}if(x==null)return 1;if(y==null)return -1;return x<y?-1:x>y?1:0;}
...
function filingsSection(sec){
  if(!sec||!sec.cik)return '';const rows=(DATA.subs||{})[String(sec.cik)]||[];if(!rows.length)return '';
  const per=sec.per||'';const after=rows.filter(r=>r.k==='fin'&&per&&r.d>per);
  const grp=k=>rows.filter(r=>r.k===k);
  const line=r=>`<div class="fl"><span class="num dim2">${esc(r.d)}</span> <a href="${esc(r.url)}" target="_blank" rel="noopener"><span class="badge">${esc(r.f)}</span></a> <span class="dim">${esc(r.desc||r.items||'')}</span>${r.k==='fin'&&per&&r.d>per?' <span class="badge nme" title="filed after the balance-sheet date; the cash above does not include it">after balance sheet</span>':''}</div>`;
  const blocks=[['fin','Financing forms (424B, S-1, S-3), 24 months'],['own','Ownership (13D / 13G), 24 months'],['8k','8-K, 12 months'],['report','Periodic reports']].map(([k,l])=>{const g=grp(k);return g.length?`<div class="lbl" style="margin:8px 0 3px">${l}</div>${g.map(line).join('')}`:'';}).join('');
  return `<div class="sec"><h4 data-gl="filings">Filings <span class="dim2" style="text-transform:none;letter-spacing:0">· SEC submissions · ${rows.length} kept${after.length?` · <b style="color:var(--warn)">${after.length} financing form${after.length>1?'s':''} after the ${esc(per)} balance sheet</b>`:''}</span></h4>${blocks}</div>`;
}
/* ---------- SEC financial row ---------- */
function secRow(sec,nTrials){
  ...
  if(rw&&rw.cash){const per=sec.per?String(sec.per).replace('CY','').replace('I',''):'';
    out+=`<span class="k">Cash + investments</span><span class="v num">${fmtUSD(rw.cash)} <span class="dim2">${esc(per)}${sec.invtags?' · '+esc(sec.invtags):sec.sti==null?' · no investment tag found':''}</span></span>`;
    if(sec.debt)out+=`<span class="k">Debt</span><span class="v num">${fmtUSD(sec.debt)} <span class="dim2">${esc(sec.debttags||'')} · not netted from cash</span></span>`;
    const basis=rw.basis==='ocfq'?`operating cash outflow, ${sec.ocfm||''}-month year-to-date ${esc(sec.ocfper||'')} annualized`:rw.basis==='ocf'?`operating cash outflow, fiscal year ${esc(sec.ocfper_fy||'2025')}`:'net loss (accounting proxy; no cash-flow figure reported)';
    if(rw.burn>0)out+=`<span class="k">Burn-implied runway</span><span class="v num">${rw.yrs>=10?'>10':rw.yrs.toFixed(1)} yrs <span class="dim2">at ${fmtUSD(rw.burn)}/yr · ${basis}</span></span>`;
    else if(rw.basis==='ocfq'||rw.basis==='ocf')out+=`<span class="k">Burn-implied runway</span><span class="v">n/a <span class="dim2">operating cash flow positive · ${basis}</span></span>`;
